...
void ConfigInit() {
    if (fs::exists("Launcher.cfg")) {
        ...
        if (cfg.is_open()) {
            ...
            if (d.HasParseError()) {
                log_fatal("Config failed to parse make sure it's valid JSON! Code : %d", d.GetParseError());
            }
            ParseConfig(d);
        } else {
            log_fatal("Failed to open Launcher.cfg!");
        }
    } else {
...
"Build": "Default"
            ...
            log_fatal("Failed to write config on disk!");
        }
    }
}
