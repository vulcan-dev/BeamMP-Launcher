...
std::string Auth(SOCKET Sock) {
    ...
    if (Res.empty() || Res == "-") {
        log_info("Didn't Receive any mods...");
        ListOfMods = "-";
        TCPSend("Done", Sock);
        log_info("Done!");
        return "";
...
char *TCPRcvRaw(SOCKET Sock, uint64_t &GRcv, uint64_t Size) {
    ...
    do {
        ...
        if (Temp < 1) {
            log_info(reinterpret_cast<const char *>(Temp));
...
void InvalidResource(const std::string &File) {
    ...
    log_warn("The server tried to sync \"%s\" that is not a .zip file!", File.c_str());
    Terminate = true;
}
    ...
    for (auto FN = FNames.begin(), FS = FSizes.begin(); FN != FNames.end() && !Terminate; ++FN, ++FS) {
        ...
        if (fs::exists(a)) {
            ...
            if (fs::file_size(a) == std::stoull(*FS)) {
                UpdateUl(false,
                         std::to_string(Pos) + "/" + std::to_string(Amount) + ": " + a.substr(a.find_last_of('/')));
                ...
                try {
                    ...
                    log_error("Failed copy to the mods folder! %s", e.what());
                    Terminate = true;
                    continue;
                }
                WaitForConfirm();
